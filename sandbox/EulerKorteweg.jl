...
mutable struct EulerKorteweg_GP <: AbstractModel
    ...
    function EulerKorteweg_GP(
        ...
        function mapfro(U)
            fftψ = U[1]
            return (abs.(ifft(fftψ)) .^ 2 .- 1) / ϵ, κ / ϵ * imag((ifft(∂ₓ .* fftψ)) .* conj(ifft(fftψ))) ./ (abs.(ifft(fftψ)) .^ 2),
                #κ*real.(ifft(∂ₓ.*fft(angle.(ifft(U))))),
                mesh.x
        end

        return new(label, f!, D, g!, mapto, mapfro, info)
    end
end
...
mutable struct EulerKorteweg_Grenier <: AbstractModel
    ...
    function EulerKorteweg_Grenier(
            param::NamedTuple;
            mesh = Mesh(param),
            dealias = 0, smooth = false,
            ktol = 0,
        ...
        function mapfro(U)
            ...
            return (abs.(ifft(fftψ)) .^ 2 .- 1) / ϵ, κ / ϵ * imag((ifft(∂ₓ .* fftψ)) .* conj(ifft(fftψ))) ./ (abs.(ifft(fftψ)) .^ 2), mesh.x
        end
