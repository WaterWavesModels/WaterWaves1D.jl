...
function create_animation(
        problems; name = nothing, x = nothing, Nframes = 201,
    ...
    anim = @animate for l in L
        ...
        for i in eachindex(problems)
            plot_solution!(
                plt, problems[i]; t = problems[i].times.ts[l], x = x,
    ...
    if !isnothing(name)
        gif(anim, string(name, ".gif"), fps = 15)
    end
    return anim
end
...
function plot_solution!(plt, problems; t = nothing, x = nothing, interpolation = false, compression = false, surface = true, fourier = true, velocity = false, label = nothing)
    ...
    for i in eachindex(problems)
        ...
        if fourier
            if !all(isnan, fftη)
                plot!(
                    plt[n, 1], fftshift(Mesh(y).k)[indices],
                    eps(1.0) .+ abs.(fftshift(fftη))[indices];
...
function plot_difference!(plt, problems; t = nothing, x = nothing, interpolation = false, compression = false, fast = false, surface = true, fourier = false, velocity = false, label = nothing)
    ...
    for i in eachindex(pairs)
        ...
        if surface
            plot!(
                plt[n, 1], X1[indices], (η1 - η2)[indices];
        ...
        if velocity
            plot!(
                plt[n, 1], X1[indices], (v1 - v2)[indices];
        ...
        if fourier
            if !all(isnan, fftη1 - fftη2)
                plot!(
                    plt[n, 1], fftshift(Mesh(x1).k)[indices],
                    eps(1.0) .+ abs.(fftshift(fftη1 - fftη2))[indices];
                    title = "Fourier coefficients (log scale)",
                    #yaxis="log scale",
                    yscale = :log10,
                    label = lbl
