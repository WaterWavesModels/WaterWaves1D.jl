...
pBBM0 = Problem(HBBM(param; dealias = 1, prep = 0), init, param)
pBBM1 = Problem(HBBM(param; dealias = 1, prep = 1), init, param)
pBBM2 = Problem(HBBM(param; dealias = 1, prep = 2), init, param)
...
plot((pBBM, pBBM0); T = 0.5, var = :velocity)
plot((pBBM, pBBM1); T = 0.5, var = :velocity)
plot!((pBBM, pBBM2); T = 0.5, var = :velocity)

u, v, w, x = solution_full(pBBM1; T = 0.5);
u₂, v₂, w₂, x₂ = solution_full(pBBM2; T = 0.5);

_, uB, = solution(pBBM; T = 0.5);
...
function Diff(; δ, ϵ, prep = 1, dt = 1.0e-4, T = 0.5, L = 10, N = 2^9, p = 2)
    param = (
        δ = δ,
        ϵ = ϵ,
...
eps = 10 .^ (-(1:0.5:4))
for ϵ in eps
    push!(N, Diff(δ = δ, ϵ = ϵ, prep = prep, dt = 1.0e-5, L = 100, N = 2^11, p = Inf))
end
...
function Normfast(; δ, ϵ, prep = 1, dt = 1.0e-4, T = 0.5, L = 10, N = 2^9, p = 2)
    param = (
        δ = δ,
        ϵ = ϵ,
        N = N, # number of collocation points
...
eps = 10 .^ (-(1:0.5:4))
for ϵ in eps
    push!(N, Normfast(δ = δ, ϵ = ϵ, prep = prep, dt = 1.0e-5, T = 0.1))
end
