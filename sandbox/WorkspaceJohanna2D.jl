...
function ∂fy(f)
    return real.(ifft(∂y .* fft(f, 2), 2))
end
function ∂fx(f)
    return real.(ifft(∂x .* fft(f, 1), 1))
end
...
function IntegrateSV(; ϵ = 1, L = π, N = 2^9, T = 0.1, dt = 1.0e-3, dealias = 1, smooth = false, Ns = nothing, hamiltonian = false)
    if isnothing(Ns)
        params = (
            ϵ = ϵ,
            N = N, L = L,
...
function Convergence(smooth = false, hamiltonian = false)
    ...
    Nref = 2^10
    reference_problem = IntegrateSV(L = π, N = Nref, T = 0.1, dt = 1.0e-4, Ns = 1, smooth = smooth, hamiltonian = hamiltonian)
    ...
    for n in 6:9
        p = IntegrateSV(L = π, N = 2^n, T = 0.1, dt = 1.0e-4, Ns = 1, smooth = smooth, hamiltonian = hamiltonian)
        push!(problems, p)
