...
@gif for time in LinRange(0, 2, 101)
    plt = plot([pFG3a, pGN], var = [:differences], T = time, legend = :bottomleft, xlims = (0, 10), ylims = (-1.0e-5, 1.0e-5))
    plot!(plt, [pFG3b, pGN], var = [:differences], T = time, legend = :bottomleft, xlims = (0, 10), ylims = (-1.0e-5, 1.0e-5))
...
function norms(p; T = nothing, a, μ, s = 0, rel = false)
    ...
    j = 0
    if rel == true
        N0 = sum(norm(p; T = T[1], a = a, μ = μ, s = s))
    else
        N0 = 1
    end
...
function norms(p1, p2; T = nothing, a, μ, rel = false)
    ...
    j = 0
    if rel == true
        N0 = sum(norm(p1, p2; T = T[1], a = a, μ = μ))
    else
        N0 = 1
    end
