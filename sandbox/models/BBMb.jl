...
- `label`: a label for future references (default is `"BBMb-Boussinesq"`);


# Return values
Generate necessary ingredients for solving an initial-value problem via `solve!`:
1. a function `BBMb.f!` to be called in explicit time-integration solvers;
2. a function `BBMb.mapto` which from `(η,v)` of type `InitialData` provides the raw data matrix on which computations are to be executed.
3. a function `BBMb.mapfro` which from such data matrix returns the Tuple of real vectors `(η,v,x)`, where
