...
struct Mesh
    ...
    function Mesh(param::NamedTuple)
        ...
        return new(N, xmin, xmax, dx, x, kmin, kmax, dk, k)
...
struct Times
    ...
    function Times(dt, tfin)
        ...
        nr = 1
        tr = t
        return new(Nt, Nr, nr, tfin, dt, t, tr)
    end
...
function (m::Matsuno)(U::Array{ComplexF64, 2})
    ...
    return @inbounds for i in eachindex(m.unew)
...
function create_animation(mesh, times, data)
    ...
    anim = @animate for l in 1:size(data)[end]
        ...
        plot!(
            pl[2, 1], fftshift(mesh.k),
            log10.(1.0e-18 .+ abs.(fftshift(fft(hr))));
            title = "frequency",
            label = model.label
        )
