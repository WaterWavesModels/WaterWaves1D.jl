...
function ColdHot(; scenario)
    ...
    @time model = WhithamGreenNaghdi(param2; SGN = true, dealias = 0, precond = false, gtol = 1.0e-12)
...
function Wavebreaking1()
    ...
    @time solve!(problem)
    p = plot(layout = (2, 1))
    plot_solution!(p, problem, 0.03898)
...
function Wavebreaking2()
    ...
    @time solve!(problem)
    p = plot(layout = (2, 1))
    plot_solution!(p, problem, t = 0.92)
