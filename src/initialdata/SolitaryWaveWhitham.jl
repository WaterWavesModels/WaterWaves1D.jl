...
function SolitaryWaveWhitham(
    ...
    function F(u::Vector{Float64})
        return -c * u + real.(ifft(F₁ .* fft(u))) + 3 * ϵ / 4 * u .^ 2
    end
    function Fabs(u::Vector{Float64})
        return c * abs.(u) + abs.(ifft(F₁ .* fft(u))) + 3 * ϵ / 4 * u .^ 2
    end
    ...
    if iterative == false
        ...
        function JacF(v, dxv)
            return Symmetric(M + Diagonal(3 * ϵ / 2 * v .- c) + α * dxv * dxv')
        end
    else
