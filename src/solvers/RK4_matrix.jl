export RK4_matrix,RK4_matrix_naive
export step!

"""
    RK4_matrix(arguments;realdata)
...
"""
struct RK4_matrix <: TimeSolver

    U1 :: Array
    dU :: Array
    label :: String

    function RK4_matrix( U :: Array; realdata=nothing )
        U1 = copy(U)
        dU = copy(U)
        if realdata==true
        ...
        new( U1, dU, "RK4_matrix")
    end

    function RK4_matrix( model :: AbstractModel; realdata=nothing )
        U=model.mapto(Init(x->0*x,x->0*x))
        RK4_matrix( U; realdata=realdata)
    end
    function RK4_matrix( param::NamedTuple, datasize=2::Int; realdata=nothing )
        RK4_matrix( zeros(Complex{Float64}, (param.N,datasize)) ; realdata=realdata)
    end
end


function step!(s  :: RK4_matrix,
                m :: AbstractModel,
                U  ,
                dt )
...
A naive version of `RK4_matrix`, without argument since no pre-allocation is performed.

"""
struct RK4_matrix_naive <: TimeSolver

    label :: String

    function RK4_matrix_naive() new("RK4_matrix (naive)") end
end

function step!(s  :: RK4_matrix_naive,
               m :: AbstractModel ,
               U  ,
               dt )
