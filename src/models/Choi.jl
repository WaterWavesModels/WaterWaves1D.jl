...
mutable struct Choi <: AbstractModel
    ...
    function Choi(
            param::NamedTuple; M = 2, reg = false,
            mesh = Mesh(param),
            dealias = 0,
            ktol = 0,
            iterate = true,
        ...
        function regul(f)
            ...
            return ifft(min.(1, 1 ./ (μ * k .^ 2)) .* fft(f))
        end
        ...
        function f!(U)
            ...
            elseif iterate == true
                ...
                fftu .= gmres(
                    LinearMap(F2, length(h); issymmetric = false, ismutating = false), q;
        ...
        function mapto(data::InitialData)
            ...
            elseif iterate == true
                ...
                fftu .= gmres(
                    LinearMap(F2, length(h); issymmetric = false, ismutating = false), fft(data.v(x));
                    restart = restart, maxiter = maxiter, Pl = Precond, reltol = gtol
                )
            end
        ...
        function mapfro(U)
            ...
            return real(ifft(U[1])), real(ifft(f2(h, fftv))), mesh.x
        end
        ...
        function mapfrofull(U)
            ...
            return real(ifft(U[1])), real(ifft(f2(h, fftv))), real(ifft(f1(h, fftv)) ./ h), real(ifft(fftv))
        end
