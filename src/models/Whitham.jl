...
struct Whitham <: AbstractModel
    ...
    function Whitham(
            param::NamedTuple; KdV = false, BBM = false,
        ...
        function mapto(data::InitialData)
            ...
            U = [
                fftr - ϵ₀ / 4 * Π⅔ .* fft(ifft(∂ₓ .* fftr) .* ifft(∂ₓ⁻ .* ffts) + r .* s + 1 / 2 * s .^ 2),
        ...
        function mapfro(U)
            ...
            return real(ifft(newr + news)), real(ifft((newr - news) ./ F₁)), mesh.x
        end
