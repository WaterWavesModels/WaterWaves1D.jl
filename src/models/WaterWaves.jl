...
"""
struct WaterWaves <: AbstractModel
    ...
    function WaterWaves(
        ...
        function mapto(data::InitialData)
            if data.η(x) == zero(x) && data.v(x) == zero(x)  #useful when defining the type of initial data in some solvers
                ...
                function JacF(u)
                    ...
                    fη .= fft(data.η(x + ϵ / ν * dx))
                    δ = meanf(fη)
                    dF(φ) = φ + ϵ * sqrt(μ) * 1im * Π⅔ .* (
                        cotanh(sqrt(μ) * (1 + ϵ * δ) * k) .* fft(dη(x + ϵ / ν * dx) .* ifft(φ))
                ...
                if method == 3
                    ...
                    function JacFMat(u)
                        ...
                        fη .= fft(data.η(x + ϵ / ν * dx))
                        δ = meanf(fη)
                        return Id + ϵ * sqrt(μ) * 1im * Π⅔ .* (
                            M(cotanh(sqrt(μ) * (1 + ϵ * δ) * k)) * FFT * M(dη(x + ϵ / ν * dx)) * IFFT
        ...
        function f!(U)
            ...
            return U[1] .= Π⅔ .* fft((1 .+ ϵ * Dxv) .* M1 ./ J - ϵ * Dz .* M2 + ϵ * q0 * Dz) / ν
        ...
        function f1!(U1, U2)
            ...
            return U1 .= Π⅔ .* fft((1 .+ ϵ * Dxv) .* M1 ./ J - ϵ * Dz .* M2 + ϵ * q0 * Dz) / ν
        end
        function f2!(U1, U2)
            Dphi .= real.(ifft(U2))
            ξ .= sqrt(μ) * (1 .+ ϵ * meanf(U1)) * k
            ...
            return U2 .= -∂ₓ .* U1 - ϵ * Π⅔ .* ∂ₓ .* fft(Dphi .* M2 + 1 / 2 .* (Dphi .^ 2 - μ * M1 .^ 2) ./ J - q0 * Dphi) / ν
        end
