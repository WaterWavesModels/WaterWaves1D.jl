...
function convert(::Type{Problem}, p :: ProblemSave)
    ...
    model = try # try with set of parameters defined by the model
        getfield(WaterModels1D, p.model)(param_m)
    catch m
        if isa(m,AbstractModel) == false # if error, then use set of parameters defined by the problem
            model = getfield(WaterModels1D, p.model)(param_p)
        end
    end
    if :kwargs in fieldnames(typeof(model))
        model = getfield(WaterModels1D, p.model)(param_m;param_m.kwargs...)
    end
    ...
    solver = try
        getfield(WaterModels1D, p.solver)(param_s)
    catch s
        if isa(s,TimeSolver) == false
            solver = try
                getfield(WaterModels1D, p.solver)(model)
            catch s
                if isa(s,TimeSolver) == false
                    solver = try
                        getfield(WaterModels1D, p.solver)()
                    catch s
                        if isa(s,TimeSolver) == false
