...
for model in models
    @testset "Preserved quantities : $(model.label)" begin
        ...
        @test isapprox(mass(pb), mass(pb; T = 0), rtol = 1.0e-10)
        @test isapprox(momentum(pb), momentum(pb; T = 0), rtol = 1.0e-10)
        @test isapprox(energy(pb), energy(pb; T = 0), rtol = 1.0e-8)
        @test abs(mass_diff(pb)) < 1.0e-10
        @test abs(momentum_diff(pb)) < 1.0e-10
        @test abs(energy_diff(pb; rel = true)) < 1.0e-8
    end
end
