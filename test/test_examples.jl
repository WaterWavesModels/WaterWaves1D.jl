...
@testset "Example: ManyModelsforWaterWaves" begin
    ...
    Diff, = Integrate(
        1; μ = 0.1, ϵ = 0.1, p = 2, N = 2^8, L = 10, T = 1, dt = 0.01,
        dealias = 0, iterate = true, precond = true, method = 1, maxiter = 10,
        name = nothing
    )

    @test 1.0e-3 < Diff[1] < 2.0e-3   # SGN
    @test 1.0e-4 < Diff[2] < 4.0e-4   # WGN
    @test 1.0e-5 < Diff[3] < 4.0e-5   # IK2

    Diff, = Integrate(
        2; μ = 0.1, ϵ = 0.1, p = 2, N = 2^8, L = 10, T = 1, dt = 0.01,
        dealias = 0, iterate = true, precond = true, method = 2, maxiter = 10,
        name = nothing
    )

    @test 2.0e-3 < Diff[1] < 4.0e-3   # SGN
    @test 4.0e-4 < Diff[2] < 8.0e-4   # WGN
    @test 1.0e-4 < Diff[3] < 2.0e-4   # IK2
...
@testset "Study: Rectified WW2" begin
    ...
    @test (blowup_time, blowup) == (1.0, false)
    @test 9.0e-7 < error_energy < 10.0e-7
...
@testset "Study: Whitham-Green-Naghdi" begin
    ...
    η, v = solution(pb)
    @test norm(η - η0) / norm(η) < 2.0e-7 || norm(v - v0) / norm(v) < 4.0e-8
...
@testset "Study: SaintVenant" begin
    ...
    problem1 = IntegrateSV(; init = 1, α = 1.5, N = nothing, h₀ = nothing, v₀ = nothing, ϵ = 1, L = π, M = 2^6, T = 0.05, dt = 1.0e-5, dealias = true, smooth = false, Ns = 1)
    η, v = solution(problem1)
    @test all((norm(η), norm(v)) .≈ (1.1384447011432766, 0.13223692570972134))


    problem2 = IntegrateSV(; init = 2, α = nothing, N = nothing, h₀ = 1 / 2, v₀ = 2, ϵ = 1, L = π, M = 2^6, T = 0.05, dt = 1.0e-5, dealias = true, smooth = true, Ns = 1)
    η, v = solution(problem2)
    @test all((norm(η), norm(v)) .≈ (3.402895534611065, 11.157811912130184))

    problem3 = IntegrateSV(; init = 3, α = nothing, N = nothing, h₀ = nothing, v₀ = nothing, ϵ = 1, L = π, M = 2^6, T = 0.05, dt = 1.0e-5, dealias = false, smooth = nothing, Ns = 1)
    η, v = solution(problem3)
    @test all((norm(η), norm(v)) .≈ (1.26323755549213, 1.1565217449137426))

    problem2Da = IntegrateSV2D(;
        N = nothing, s = 2, h₀ = 1 / 2,
        u₀ = 1 / 2, v₀ = -1 / 2, u₁ = 1, v₁ = -1,
        ϵ = 1, L = π, M = 2^5, T = 0.01, dt = 5.0e-5,
