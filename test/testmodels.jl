...
init = Init(x -> exp.(-1 / 8 * x .^ 4), x -> 0.0 * x)
...
push!(
    models, relaxedGreenNaghdi(
        paramrelax::NamedTuple;
        id = 2,
        FG = true,
        dealias = 1,
        ktol = 0,
...
model1 = Boussinesq(
    param;
    a = -1 // 2, b = 5 // 12,
    dealias = 1, ktol = 1.0e-12,
    label = "Boussinesq"
)
model2 = WhithamBoussinesq(
    param;
    Boussinesq = true,
    a = -1 / 2, b = 5 / 12,
    dealias = 1,
...
push!(
    models, modifiedMatsuno(
        merge(param, (ν = 1 / √para.μ,));
        ktol = 1.0e-14, dealias = 0,
