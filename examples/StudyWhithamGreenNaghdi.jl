...
function PlotSolitaryWaveWGN1(; c = 2, N = 2^10, L = 10 * π, verbose = false, name = nothing)
    ...
    (η, u, v, mesh) = SolitaryWaveWhithamGreenNaghdi(
        param;
        method = 2, α = 0,
        tol = 1.0e-14, max_iter = 10,
        ktol = 0 * 1.0e-11, gtol = 1.0e-16,
        iterative = true, q = 1,
        verbose = verbose
    )
...
function PlotJacobianWGN(; c = 20, L = 10 * π, N = 2^10, SGN = false, verbose = false, name = nothing)
    ...
    Jac = (
        -1 / 3 * M(1 ./ hu .^ 2) * M₀ * M(hu .^ 3) * (c * M₀ .+ 3 * ϵ * M(Fu))
            .+ ϵ * M(hu .* Fu) * M₀
    ...
    plt = plot(layout = (1, 2))
    surface!(
        plt[1, 1], fftshift(k), fftshift(k)[N:-1:1],
        log10.(abs.(FFT * Jac * IFFT)),
        title = "Jacobian"
    )
    surface!(
        plt[1, 2], fftshift(k), fftshift(k)[N:-1:1],
...
function IntegrateSolitaryWaveWGN(; SGN = false, c = 2, N = 2^10, L = 10 * π, T = 1, dt = 1 / 2000, name = nothing)
    ...
    if SGN == true
        ...
        (ηinit, uinit, vinit, mesh) = SolitaryWaveWhithamGreenNaghdi(
            param; method = 3, max_iter = 10, α = 1, verbose = true, tol = 1.0e-12
        )
        (ηfin, ufin, vfin) = SolitaryWaveWhithamGreenNaghdi(
            param; x₀ = c * T, method = 3, max_iter = 10, α = 1, verbose = true, tol = 1.0e-12
        )
    end
    ...
    plt = plot(layout = (1, 2))
    plot!(
        plt[1, 1], fftshift(mesh.k), fftshift(log10.(abs.(fft(uinit))));
        title = "frequency",
        label = "initial",
        xlabel = "x",
        ylabel = "u"
    ...
    plot!(
        plt[1, 1], fftshift(mesh.k), fftshift(log10.(abs.(fft(ucomp))));
        label = "final"
    )
    plot!(
        plt[1, 2], mesh.x, ufin - ucomp;
...
function StabilitySolitaryWaveWGN(; p = 2, c = 2, N = 2^10, L = 10 * π, T = 10, dt = 10 / 10^4, SGN = false, precond = true, iterate = true, dealias = 0, name = nothing)
    ...
    if SGN == true
        ...
        (η, u, v, mesh) = SolitaryWaveWhithamGreenNaghdi(
            param; method = 3, tol = 1.0e-14, max_iter = 10, α = 1, verbose = true
        )
    end
    k = mesh.k
    μ = 1
    ...
    plt = plot(layout = (1, 2))
    plot!(
        plt[1, 1], fftshift(mesh.k), fftshift(log10.(abs.(fft(η))));
        title = "frequency",
        label = "initial"
    )
    plot!(
        plt[1, 1], fftshift(mesh.k), fftshift(log10.(abs.(fft(ηfin))));
        label = "final"
    )
    plot!(
        plt[1, 2], mesh.x, [η ηfin];
        title = "at time t=$(problem.times.tfin)",
        label = ["zeta initial" "zeta final"]
    )
    plot!(
        plt[1, 2], mesh.x, [u ufin];
...
function IntegrateWGN(scenario; δ = 0.1, N = 2^11, L = 3 * π, x₀ = -3, T = 5, dt = 5 / 10^4, SGN = false, dealias = 0, iterate = true, precond = true, name = nothing)
    ...
    init = Init(mesh, η, v)
    model = WhithamGreenNaghdi(param; SGN = SGN, ktol = 0, gtol = 1.0e-12, iterate = iterate, precond = precond, dealias = dealias)
    problem = Problem(model, init, param)
    solve!(problem)
