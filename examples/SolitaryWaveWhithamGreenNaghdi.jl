...
function solve2!(η₀,η₁,η₂,c,dc)
		tη₂ = copy(η₂)
		η₂ .= SolitaryWaveWhithamGreenNaghdi2(mesh, merge(param,(c=c,)), P.(dc,dc,η₀,η₁,η₂);
				tol =  1e-16, max_iter=5,
...
(η₀,v₀,flag) = SolitaryWaveWhithamGreenNaghdi2(
...
for cs = range(5; step = 1, stop = 25)
	print(string("c = ",cs,"\n"))
	solve2!(η₀,η₁,η₂,cs,1)
end

sη₀=copy(η₀);sη₁=copy(η₁);sη₂=copy(η₂);
η₀=copy(sη₀);η₁=copy(sη₁);η₂=copy(sη₂);


for cs = range(26; step = 1, stop = 30)
	print(string("c = ",cs,"\n"))
	solve2!(η₀,η₁,η₂,cs,1)
end
...
η₀ = real.(ifft(interpolate(fft(η₂),mesh.k)))

(η₂,v₂,flag) = SolitaryWaveWhithamGreenNaghdi(
				mesh, merge(param,(c=4,)),
				real.(ifft(interpolate(fft(η₂),mesh.k)));
				tol =  1e-16, max_iter=20,
				iterative = true,
				verbose = true, GN = false)
