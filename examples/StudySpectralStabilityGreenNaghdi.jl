...
function figspecCW(p; N = 2^6, P = 1, M = 2^6, SGN = true::Bool)
    ...
    σ = Spectrum(param2, η, u, parc.c, 1im * 0.0).values
    val = σ #[Int(7*end/8):end]
    @showprogress 1 for ν in (-π / parc.λ / P / 2):(π / parc.λ / P / M):(π / parc.λ / P / 2)
...
function figspecSW(c; N = 2^6, L = 10 * π, M = 2^6, SGN = true::Bool)
    ...
    σ = Spectrum(param, η, u, c, 1im * 0.0).values
    val = σ #[Int(7*end/8):end]
    @showprogress 1 for ν in (-π / L / 2):(π / L / M):(π / L / 2)
