...
"""
function IntegrateSV(;
        init, α = 1.5, N = nothing, h₀ = 1 / 2, v₀ = 2,
        ϵ = 1, L = π, M = 2^9, T = 0.5, dt = 1.0e-5,
...
function Convergence(; init, smooth = false, name = nothing, T = 0.5)
    ...
    if init == 1
        # Reference solution
        reference_problem = IntegrateSV(
            init = 1, α = 1.5, ϵ = 1, L = π, M = 2^15,
            T = T, dt = 1.0e-5, dealias = true,
        ...
        for n in 14:-1:6
            ...
            p = IntegrateSV(
                init = 1, α = 1.5, ϵ = 1, L = π, M = 2^n, T = T, dt = 1.0e-5,
            ...
            Nmodes = 2^(n - 1)
            Ucomp = [
                Uref[1][1:Nmodes]; Uref[1][(end - Nmodes + 1):end];
    ...
    elseif init == 2
        for n in 14:-1:6
            # Reference solution
            reference_problem = IntegrateSV(
                init = 2, N = (2^n ÷ 3), h₀ = 0.5, v₀ = 2,
                ϵ = 1, L = π, M = 2^15, T = T, dt = 1.0e-5,
                dealias = true, smooth = smooth,
                Ns = 1, label = "reference"
            )
            # Lower-resolution run
            p = IntegrateSV(
                init = 2, h₀ = 0.5, v₀ = 2, ϵ = 1, L = π, M = 2^n, T = T,
                dt = 1.0e-5, dealias = true, smooth = smooth,
                Ns = 1, label = "2M=2^$n"
            )
            # Fourier coefficients
            Uref = reference_problem.data.U[end] / 2^15
...
function Figure1()
    ...
    return savefig(Fig1b, "Fig1b.svg")
end
...
function Figure3()
    ...
    return savefig(Fig3b, "Fig3b.svg")
end
...
function Figure4()
    ...
    # Smooth low-pass filter
    pb_smooth_M10 = IntegrateSV(
        init = 2, h₀ = 0, v₀ = 2, ϵ = 1, L = π, M = 2^10, T = 0.1,
        dt = 1.0e-5, dealias = true, smooth = true, Ns = 1, label = "smooth"
    )
    # Sharp low-pass filter
    pb_sharp_M10 = IntegrateSV(
        init = 2, h₀ = 0, v₀ = 2, ϵ = 1, L = π, M = 2^10, T = 0.1,
        dt = 1.0e-5, dealias = true, smooth = false, Ns = 1, label = "sharp"
    )
    ...
    # Smooth low-pass filter
    pb_smooth_M12 = IntegrateSV(
        init = 2, h₀ = 0, v₀ = 2, ϵ = 1, L = π, M = 2^12, T = 0.1,
        dt = 1.0e-5, dealias = true, smooth = true, Ns = 1, label = "smooth"
    )
    # Sharp low-pass filter
    pb_sharp_M12 = IntegrateSV(
        init = 2, h₀ = 0, v₀ = 2, ϵ = 1, L = π, M = 2^12, T = 0.1,
        dt = 1.0e-5, dealias = true, smooth = false, Ns = 1, label = "sharp"
    )
    ...
    return savefig(Fig4b, "Fig4b.svg")
end
...
function FigureX()
    ...
    pb_sharp_M10 = IntegrateSV(
        init = 3, ϵ = 1, L = π, M = 2^10, T = 1, dt = 1.0e-5,
    ...
    pb_sharp_M12 = IntegrateSV(
        init = 3, ϵ = 1, L = π, M = 2^12, T = 1, dt = 1.0e-5,
        dealias = true, smooth = false, Ns = 100, label = "sharp"
    )
    pb_smooth_M12 = IntegrateSV(
        init = 3, ϵ = 1, L = π, M = 2^12, T = 1, dt = 1.0e-5,
    ...
    return plot([pb_sharp_M12, pb_smooth_M12], T = 1)
end
...
"""
function IntegrateSV2D(;
        N = nothing, s = 2, h₀ = 1 / 2,
        u₀ = 1 / 2, v₀ = -1 / 2, u₁ = 1, v₁ = -1,
        ϵ = 1, L = π, M = 2^9, T = 0.1, dt = 5.0e-5,
...
function Convergence2D(; smooth = false, hamiltonian = false, name = nothing, T = 0.1)
    ...
    for n in 9:-1:6
        # Reference solution
        reference_problem = IntegrateSV2D(
            M = 2^10, N = (2^n) ÷ 3, T = T,
        ...
        # L² error
        push!(
            E0, norm([norm(U1 - Ucomp1); norm(U2 - Ucomp2); norm(U3 - Ucomp3)]) /
...
function Figure5()
    ...
    savefig(Fig5b, "Fig5b.pdf")
    return savefig(Fig5b, "Fig5b.svg")
end
...
function Figure6()
    # M = 2^9
    # Smooth low-pass filter
    problem = IntegrateSV2D(
        h₀ = -0.1, u₀ = 0.5, v₀ = -0.5, u₁ = 1, v₁ = -1,
        M = 2^9, T = 0.1, dt = 5.0e-5, smooth = false,
        hamiltonian = false, Ns = 1
    )
    η, vx, vy, x, y = solution(problem)

    # Sharp low-pass filter
    problem_smooth = IntegrateSV2D(
        h₀ = -0.1, u₀ = 0.5, v₀ = -0.5, u₁ = 1, v₁ = -1,
        M = 2^9, T = 0.1, dt = 5.0e-5, smooth = true,
        hamiltonian = false, Ns = 1
    )
    ...
    problem = IntegrateSV2D(
        h₀ = -0.1, u₀ = 0.5, v₀ = -0.5, u₁ = 1, v₁ = 1,
        M = 2^10, T = 0.1, dt = 5.0e-5, smooth = false,
        hamiltonian = false, Ns = 1
    )
    η, vx, vy, x, y = solution(problem)
    # Sharp low-pass filter
    problem_smooth = IntegrateSV2D(
        h₀ = -0.1, u₀ = 0.5, v₀ = -0.5, u₁ = 1, v₁ = 1,
        M = 2^10, T = 0.1, dt = 5.0e-5, smooth = true,
        hamiltonian = false, Ns = 1
    )
    ηs, vxs, vys, = solution(problem_smooth)
    ...
    plot!(xlabel = "x", ylabel = "y")
    title!("\$\\partial_x^2 u_N, \\quad 2M = 2^{10}\$")
    savefig(Fig6d, "Fig6d.pdf")
    return savefig(Fig6d, "Fig6d.svg")
end
...
function Figure7()
    ...
    plot!(xlabel = "x", ylabel = "y")
    title!("\$\\partial_x^2 u_N, \\quad 2M = 2^{10}\$")
    savefig(Fig7d, "Fig7d.pdf")
    return savefig(Fig7d, "Fig7d.svg")
end
