...
function IntegrateWW2(; init = 1, μ = 1, ϵ = 0.1, L = 20, N = 2^10, T = 10, dt = 0.001, dealias = 1, δ = 0.001, m = -1, K = 100, p = 2, Ns = nothing)
    ...
    e(η1, v1, η2, v2) = 1 / 2 * (
        (η1 - η2) .* (η1 + η2) .+ (v1 - v2) .* ifft(tmu .* fft(v1 + v2))
...
function Figure(scenario; compression = false, name = nothing, anim = false)
    ...
    elseif scenario == 9
        ...
        plt = scatter(
            Eps, [blowups1 blowups2 blowups3 blowups4],
    ...
    elseif scenario == 11
        ...
        y2 = OHLC[(δc2[i][1], δc2[i][1], δc2[i][2], δc2[i][2]) for i in 1:length(Eps)]
        y10 = OHLC[(δc10[i][1], δc10[i][1], δc10[i][2], δc10[i][2]) for i in 1:length(Eps)]
