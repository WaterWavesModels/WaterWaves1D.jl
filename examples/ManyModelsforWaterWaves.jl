...
"""
function Integrate(scenario; μ = 0.1, ϵ = 0.1, p = 2, N = 2^9, L = 15, T = 10, dt = 0.001, dealias = 0, iterate = true, precond = true, method = 1, maxiter = 100, name = nothing)
    #---- preparation
    ...
    if typeof(dealias) == Int64
        dealias = dealias * ones(Int64, 4)
    end
    ...
    modelSGN = SerreGreenNaghdi(param; ktol = 0, gtol = 1.0e-14, iterate = iterate, precond = precond, dealias = dealias[2])
    modelWGN = WhithamGreenNaghdi(param; SGN = false, ktol = 0, gtol = 1.0e-14, iterate = iterate, precond = precond, dealias = dealias[3])
    modelIK2 = IsobeKakinuma(param; ktol = 0, gtol = 1.0e-14, iterate = iterate, precond = precond, dealias = dealias[4])
    ...
    if !isnothing(name)
        ...
        plot(
            xww, [ηww - ηSGN ηww - ηWGN ηww - ηIK2],
            color = [:2 :3 :4], primary = false
        )
        scatter!(
            xww[ind], [ηww[ind] - ηSGN[ind] ηww[ind] - ηWGN[ind] ηww[ind] - ηIK2[ind]],
